...
class DatasetReader:
    ...
    def __init__(
        ...
        # Optional decode filter: None loads all video cameras (default); otherwise only the listed ones.
    ...
    def _resolve_video_keys_to_load(self, video_keys_to_load: list[str] | None) -> list[str]:
        if video_keys_to_load is None:
            return list(self._meta.video_keys)

        requested = list(video_keys_to_load)
        unknown = sorted(set(requested).difference(self._meta.video_keys))
        if unknown:
            raise ValueError(
                f"Unknown video_keys_to_load: {unknown}. "
                f"Available video keys: {list(self._meta.video_keys)}"
            )
    ...
    def get_item(self, idx) -> dict:
        ...
        if self._image_transforms is not None:
            # Skip video cameras that were filtered out by video_keys_to_load (not decoded above).
            image_keys = [
                cam
                for cam in self._meta.camera_keys
                if cam not in self._meta.video_keys or cam in self.video_keys_to_load
            ]
