...
def test_batch_to_transition_preserves_only_generic_source_metadata():
    batch = {
        OBS_STATE: torch.randn(1, 7),
        ACTION: torch.randn(1, 4),
        "action_supervision": torch.tensor([True]),
        "latent_supervision": torch.tensor([False]),
    ...
    assert "action_supervision" not in comp_data
    assert "latent_supervision" not in comp_data
    assert comp_data["dataset_source_name"] == batch["dataset_source_name"]

    reconstructed = transition_to_batch(transition)

    assert "action_supervision" not in reconstructed
    assert "latent_supervision" not in reconstructed
